...
try {
  ...
  await page.mouse.wheel(0, 400); await page.waitForTimeout(560)
  await page.mouse.wheel(0, 400)
  await page.waitForFunction(() => Number(document.querySelector('.morph-camera')?.dataset.targetFrame) >= 100)
  await page.screenshot({ path: '/tmp/render-morph-camera-turn.png' })
  await page.mouse.wheel(0, -400)
  await page.waitForFunction(() => Number(document.querySelector('.morph-camera')?.dataset.targetFrame) <= 10)
  await page.mouse.wheel(0, 400)
  await page.waitForFunction(() => Number(document.querySelector('.morph-camera')?.dataset.targetFrame) >= 100)
  await page.mouse.wheel(0, 400); await page.waitForTimeout(560)
  await page.mouse.wheel(0, 400); await act(page, 1)
  ...
  await page.locator('.scroll-morph').focus()
  ...
  assert(end > 0, 'Excess input is released into document immediately')
  assert.equal(await page.locator('#after-scroll-morph + section').getAttribute('id'), 'portafolio')
  await page.mouse.wheel(0, 350); await page.waitForTimeout(500)
  assert((await page.evaluate(() => scrollY)) > end, 'Document scroll released at end')
